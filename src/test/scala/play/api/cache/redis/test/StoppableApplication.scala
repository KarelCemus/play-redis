...
trait StoppableApplication extends ApplicationLifecycle {
  ...
  private var hooks: List[() => Future[_]] = Nil
  ...
  def runAsyncInApplication(block: => Future[Assertion])(implicit ec: ExecutionContext): Future[Assertion] = {
    block.map(Success(_)).recover(Failure(_))
  ...
  final override def addStopHook(hook: () => Future[_]): Unit =
    hooks = hook :: hooks

  final override def stop(): Future[_] = Future.unit
