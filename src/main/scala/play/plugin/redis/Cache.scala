 ...
 */
class Cache extends ExtendedCacheAPI {

  protected var internal: ExtendedCacheAPI = reload( )

  /** looks up ExtendedCachePlugin and reassigns it into internal variable */
  def reload( ): ExtendedCacheAPI = {
    // reload cache api
    internal = Play.current.plugin[ ExtendedCachePlugin ] match {
