...
sealed trait AbstractRedisSslSettings extends Product with Serializable {
  def verifyPeerMode: Option[VerifyPeerMode]
  def toOptions: Option[SslOptions]
  def enabled: Boolean
}

sealed trait BasicRedisSslSettings extends AbstractRedisSslSettings {
  override def toOptions: Option[SslOptions] = None
}

sealed trait ExtendedRedisSslSettings extends AbstractRedisSslSettings {
...
object AbstractRedisSslSettings extends ConfigLoader[AbstractRedisSslSettings] {
  ...
  object VerifyPeerMode {
    case object NONE extends VerifyPeerMode("none", SslVerifyMode.NONE)
  ...
  def getOpt(config: Config, path: String): Option[AbstractRedisSslSettings] = {
    val pathToObject = path / "ssl-settings"

    if (config.hasPath(pathToObject)) {
      if (config.hasPath(pathToObject / "enabled")) {
        Some(
          BasicRedisSslSettingsImpl(
            enabled = config.getBoolean(pathToObject / "enabled"),
            verifyPeerMode = VerifyPeerMode.getOpt(config, pathToObject / "verify-peer-mode"),
          ),
        )
      } else {
        Some(
          ExtendedRedisSslSettingsImpl(
  ...
  object BasicRedisSslSettingsImpl {}

  final case class ExtendedRedisSslSettingsImpl(
  ...
  ) extends ExtendedRedisSslSettings

  object ExtendedRedisSslSettingsImpl {
    ...
    object SslResource {
      ...
      def get(config: Config, path: String): SslResource =
        getOpt(config, path).getOrElse(throw new IllegalArgumentException("Unknown resource. Only `file` and `url` are supported."))
    ...
    object KeyManagerDefinition {
      ...
      def get(config: Config, path: String): KeyManagerDefinition =
        getOpt(config, path).getOrElse(throw new IllegalArgumentException("Unknown key manager definition. Only `factory` and `file` are supported."))
    ...
    object TrustManagerDefinition {
      ...
      def get(config: Config, path: String): TrustManagerDefinition =
        getOpt(config, path).getOrElse(throw new IllegalArgumentException("Unknown trust manager definition. Only `factory` and `file` are supported."))

    }

  }

  override def load(config: Config, path: String): AbstractRedisSslSettings =
