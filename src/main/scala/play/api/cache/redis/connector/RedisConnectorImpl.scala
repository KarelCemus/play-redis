...
private[ connector ] class RedisConnectorImpl @Inject()( serializer: AkkaSerializer, configuration: RedisConfiguration, lifecycle: ApplicationLifecycle )( implicit system: ActorSystem ) extends RedisConnector {
  ...
  private def setTemporally( key: String, value: String, expiration: Duration ): Future[ Unit ] =
    redis.setEX( key, value, expiration.toSeconds.toInt ) executing "SETEX" withKey key andParameters s"$value $expiration" expects {
  ...
  def listRemove( key: String, value: Any, count: Int ) =
    encode( key, value ).flatMap( redis.lRem( key, _, count ) ) executing "LREM" withKey key andParameters s"$value $count" expects {
  ...
  def listInsert( key: String, pivot: Any, value: Any ) = for {
    ...
    result <- redis.lInsert( key, Position.Before, pivot, value ) executing "LINSERT" withKey key andParameter s"$pivot $value" expects {
  ...
  def setMembers[ T: ClassTag ]( key: String ) =
    redis.sMembers( key ) executing "SMEMBERS" withKey key expects {
      case items =>
  ...
  def hashValues[ T: ClassTag ]( key: String ) =
    redis.hVals( key ) executing "HVALS" withKey key expects {
